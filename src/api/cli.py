...
def cli(mode: Mode = Mode.BATCH) -> argparse.Namespace:
    """Parse command line arguments for the requested pipeline."""
    parser = argparse.ArgumentParser(description="NBA Clutch Impact CLI")
    ...
    if mode == Mode.BATCH:
        ...
        parser.add_argument(
            ...
            type=str,
            default="2024-25",
            help="Training season"
        )
        parser.add_argument(
            "--test-season",
            type=str,
            default="2025-26",
            help="Evaluation season"
        )
        parser.add_argument(
            "--refresh",
            action="store_true",
        ...
        parser.add_argument(
            "--dry-run",
            action="store_true",
            help="Run batch pipeline without writing to SQLite or saving the model",
        )
    elif mode == Mode.STREAMING:
        parser.description = "NBA clutch streaming pipeline"
        parser.add_argument("--game_id", type=str, required=True, help="Game ID")
        parser.add_argument(
            "--topic",
            type=str,
            required=False,
            default="play-by-test",
